    ...
    methods: {
      ...
      _collectionBook () {
        if (this.isCollection) {
          collectionBook(this.bookDetail.id, 0).then((res) => {
          ...
          collectionBook(this.bookDetail.id, 1).then((res) => {
            if (res.data.status === ERR_OK) {
              this.isCollection = !this.isCollection
              this.maskText = '收藏成功'
              this.$refs.dialogMask.show()
      ...
      seriesBook () {
        this.$router.push({
          path: '/chooseBooks/seriesBook=' + this.bookDetail.categoryId
        })
      },
      _isbook () {
        console.log(111)
      ...
      addbook () {
        ...
        console.log(this.books, this.bookDetail)
        let findIndex = this.findIndex(this.books, this.bookDetail)
        console.log(findIndex)
        if (findIndex > -1 && this.bookDetail.isaddcart !== 0) {
          ...
          addBorrowstand(this.bookDetail.id).then((res) => {
            console.log(res)
            if (res.data.status === ERR_OK) {
